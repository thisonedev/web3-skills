...
Words to watch: ship (use publish, release, roll out). drift (use go stale, fall out of sync). land (use ends up, arrives, sits). lock in (use finalize, confirm, decide on). clean or cleanly as a placeholder. stand up. plus as a list separator. fine as approval. print, prints as a placeholder for outputs, appears, displays: casual trading-floor slang ("I printed $1284 on this trade") bleeding into technical writing. tape (use record, log, capture) as a vague placeholder for recording or verifying something, including ticker-tape trading slang ("check the tape"). Exempt when it names a real SDK or library term, such as the npm `tape` test runner or TensorFlow's GradientTape. yields (use produces, gives, results in) when nothing in the sentence actually generates or iterates. Exempt when the subject is a real generator, iterator, or stream that literally yields values, as in "the async generator yields each chunk." flip (a flag/switch) (use set, pass, enable, turn on) when describing a code-level parameter or config option. Exempt for an actual UI toggle, checkbox, or feature-flag switch a person clicks. carries into, carries over into (use extends into, continues into, applies to, repeats in) when describing how a pattern, structure, or behavior spans two contexts without saying what actually connects them. carries, bare (use has, contains, already includes) when standing in for "contains" or "already has" a specific named value on a single concrete object. Distinct from the "carries into" sense above (two contexts) and from A1's significance sense (carries weight, implications, risk, irony). throws, it throws (bare, without naming the exception or error). Name the specific exception or error type (a TypeError, a ValueError, a custom error class) instead. Exempt when the exception type immediately follows, as in "throws a TypeError if the input is empty." comes from, goes inside, lives in, sits in (as motion or location verbs standing in for the real relationship between a non-physical code entity, such as a callback, a value, or a config, and where it's defined or used) → name the actual relationship instead (defined in, copied from, passed as, is a property of, follows the pattern in). Exempt for a real physical or file-system location ("the config lives in `/etc/app.conf`" names a real path). guess, best guess, top guess (as a placeholder for a deterministic top result, match, or highest-scoring item, when nothing in the computation is actually predicting or guessing) → use top result, top match, highest-scoring result. Exempt when the value genuinely comes from a classifier, ASR system, LLM, or other probabilistic model output, where "guess" is standard domain language (an image classifier's top-1 guess, an ASR system's n-best guesses, an LLM's top-ranked token or completion). wire, wire up, wired (as a placeholder for connect, register, attach, bind, or subscribe) → name the actual mechanism. Exempt for a real electrical/hardware connection or a literal wire protocol/wire format term.

Why this is wrong: the verb sounds like a stand-in for a specific action. The next reader has to translate the placeholder back into a real verb. "Carries into" is the vaguest of the set: it asserts a connection between two things without saying whether it's inheritance, repetition, causation, or something else, so the reader can't tell what actually happened. A bare "carries" drops that cross-context ambiguity but still substitutes for the plainer "has" or "contains." A bare "it throws" is the same failure applied to error handling: the reader still doesn't know what condition to catch. "Comes from" and "goes inside" go a step further than vagueness: they borrow physical motion for something that has no physical existence, so the code reads as if it walks between locations on its own instead of being written and placed by a developer. "Guess" cuts the other way: applied to a deterministic sort over known scores, it invents uncertainty that isn't there, when the real failure is overclaiming rather than underspecifying. "Wire" and "wired" borrow an electrical image for any kind of connection, so the reader never learns whether it's a function call, an event subscription, or a dependency-injection binding.

Before:
> The new feature ships next week. The release lands on a Tuesday. The naming structure from the config file carries into the CLI flags. The callback comes from the quickstart example and goes inside the options object that already carries `modelSrc`. If the input is malformed, it throws. The results are sorted by score descending, so `results[0]` is the top guess. The handler is wired up to the download event.

After:
> The new feature releases next Tuesday. The CLI flags reuse the config file's naming: `max_retries` becomes `--max-retries`. The callback follows the quickstart example's pattern, passed as a property of the options object that already has `modelSrc`. If the input is malformed, the parser throws a `SyntaxError`. The results are sorted by score descending, so `results[0]` is the top match. The handler subscribes to the download event.
