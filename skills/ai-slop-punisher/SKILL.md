...
- **call, the call** (the default noun/verb for a function or API invocation) → reduce it; use invoke, run, execute, request, or name the function directly, following classic programming-tutorial style (freeCodeCamp and similar). Exempt in Solidity/EVM content, where "call," "caller," "callable," and "calldata" are fixed technical terms tied to real language mechanics (an external call, `msg.sender` as the caller) and must not be swapped. Also exempt in the fixed code lead-in phrase "you'd call it like this/so" introducing a code block (see What NOT to Flag); that's a functional transition, not the reflexive noun usage this entry targets.

Why this is wrong: the verb sounds like a stand-in for a specific action. The next reader has to translate the placeholder back into a real verb. "Carries into" is the vaguest of the set: it asserts a connection between two things without saying whether it's inheritance, repetition, causation, or something else, so the reader can't tell what actually happened. A bare "carries" drops that cross-context ambiguity but still substitutes for the plainer "has" or "contains." A bare "it throws" is the same failure applied to error handling: the reader still doesn't know what condition to catch. "Comes from" and "goes inside" go a step further than vagueness: they borrow physical motion for something that has no physical existence, so the code reads as if it walks between locations on its own instead of being written and placed by a developer. "Guess" cuts the other way: applied to a deterministic sort over known scores, it invents uncertainty that isn't there, when the real failure is overclaiming rather than underspecifying. "Wire" and "wired" borrow an electrical image for any kind of connection, so the reader never learns whether it's a function call, an event subscription, or a dependency-injection binding. "Call" is different from the rest of this list: it isn't vague, it's just the model's default reach outside Solidity, where it crowds out invoke/run/execute/request and the option of just naming the function.
