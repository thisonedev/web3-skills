...
Why this is wrong: the verb sounds like a stand-in for a specific action. The next reader has to translate the placeholder back into a real verb. "Carries into" is the vaguest of the set: it asserts a connection between two things without saying whether it's inheritance, repetition, causation, or something else, so the reader can't tell what actually happened. A bare "carries" drops that cross-context ambiguity but still substitutes for the plainer "has" or "contains." A bare "it throws" is the same failure applied to error handling: the reader still doesn't know what condition to catch. "Comes from" and "goes inside" go a step further than vagueness: they borrow physical motion for something that has no physical existence, so the code reads as if it walks between locations on its own instead of being written and placed by a developer.

Before:
> The new feature ships next week. The release lands on a Tuesday. The naming structure from the config file carries into the CLI flags. The callback comes from the quickstart example and goes inside the options object that already carries `modelSrc`. If the input is malformed, it throws.

After:
> The new feature releases next Tuesday. The CLI flags reuse the config file's naming: `max_retries` becomes `--max-retries`. The callback follows the quickstart example's pattern, passed as a property of the options object that already has `modelSrc`. If the input is malformed, the parser throws a `SyntaxError`.
